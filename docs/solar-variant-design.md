...
- **DEV mode cannot see real solar.** USB serial puts 5 V on the same pin the panel feeds, so the Schottky blocks the panel and the INA219 reads ~0 mA on a 5 V bus. Serial logs and real solar are mutually exclusive by construction. Consistent with the decision not to test on-device, but it means the DEV path can never validate the solar policy.
